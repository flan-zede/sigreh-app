...
@Component({
  ...
  <form [formGroup]='form' *ngIf='form'>
    <div class='row'>
      ...
      <div class='col-lg-6 col-md-12'>
        <mat-slide-toggle formControlName='autonomy'></mat-slide-toggle>&nbsp;{{ 'autonomy'|translate }}
      </div>
    </div>
  </form>
  `
...
export class DistrictDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.route.id) {
      ...
      this.api.findOne(this.route).pipe(first())
        .subscribe(
          (item: District) => { this.form.patchValue(item); this.loader = false; },
  ...
  createForm(): void {
    this.form = this.fb.group({
      ...
      autonomy: [false, Validators.compose([Validators.required])]
    });
  }
