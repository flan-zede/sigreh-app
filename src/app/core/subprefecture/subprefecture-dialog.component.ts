...
@Component({
  selector: 'app-subprefecture-dialog',
  template: `
...
export class SubprefectureDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    forkJoin({
      ...
      .subscribe(res => { this.departments = res.departments; });
    ...
    if (this.route.id) {
      ...
      this.api.findOne(this.route).pipe(first())
        .subscribe(
          (item: Subprefecture) => { this.form.patchValue(item); this.loader = false; },
