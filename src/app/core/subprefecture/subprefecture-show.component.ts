...
@Component({
  selector: 'app-subprefecture-show',
  template: `
...
export class SubprefectureShowComponent implements OnInit {

  item = new Subprefecture();
  loader: boolean;
  route = ROUTE;
  ...
  ngOnInit(): void {
    if (this.route.id) {
      ...
      this.api.findOne(this.route).pipe(first())
        .subscribe(
          (item: Subprefecture) => { this.item = item; this.loader = false; },
